...
const RenderItem = ({result, index, onDetail}) => {
  ...
  return (
    <Fragment>
      ...
      <TouchableOpacity
        key={index}
        style={loader ? styles.default : styles.result}
        onPress={() => onDetail(result.item)}>
        <FastImage
          style={styles.image}
          source={{uri: RES_URL + result.item.restaurant_coverImage}}
        ...
        {!loader && (
          <Fragment>
            ...
            {(parseInt(moment().format('HH:mm').replace(':', '')) <=
              parseInt(result.item.restaurant_open.replace(':', '')) ||
              parseInt(moment().format('HH:mm').replace(':', '')) >=
            ...
            <View style={styles.titleView}>
              <Text
                style={[
                  ...
                  (parseInt(moment().format('HH:mm').replace(':', '')) <=
                    ...
                    parseInt(moment().format('HH:mm').replace(':', '')) >=
                      parseInt(
                        result.item.restaurant_close.replace(':', ''),
                      )) &&
                    styles.disabled,
                ]}
                numberOfLines={1}>
                {result.item.restaurant_name}
              </Text>
              <View style={styles.rating}>
                ...
                <Text style={styles.rate}>
                  {result.item.restaurant_rating}/5
                </Text>
              </View>
            </View>
            <Text
              ...
              numberOfLines={2}>
              {result.item.restaurant_description}
            </Text>
          </Fragment>
        )}
      </TouchableOpacity>
      ...
      {!isEmpty(props.data) ? (
        ...
        <View style={styles.emptyView}>
          <Text style={styles.cardTitle}>
            {i18n.translate('No restaurant found')}
          </Text>
        </View>
      )}
    </Card>
