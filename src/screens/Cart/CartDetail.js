      ...
      <View style={styles.cartBottom}>
        <View style={styles.cartLeft}>
          ...
          <Text style={styles.price}>
            {cartFinalPrice().toFixed(2)} {i18n.translate('lei')}
          </Text>
        </View>
...
const UpSellProductItem = ({upSellProduct, index, onSelectUpSellProduct}) => {
...
export default CartDetail = props => {
  ...
  useEffect(() => {
    ...
    (visitHouseNumber && !validateBetween(addressHouseNumber, 1, 20))
      ...
      : setErrorHouseNumber('');
    (visitName && isEmpty(userName)) || (visitName && !validateName(userName))
      ? setErrorName(
          i18n.translate('The name must be at least 3 characters long'),
        )
      : setErrorName('');
    (visitPhone && isEmpty(phone)) || (visitPhone && !validateMobile(phone))
      ? setErrorPhone(i18n.translate('Mobile is not valid'))
      : setErrorPhone('');
    (visitEmail && isEmpty(email)) || (visitEmail && !validateEmail(email))
      ? setErrorEmail(i18n.translate('Email is not valid'))
      : setErrorEmail('');
  }, [
    addressStreet,
    visitStreet,
  ...
  useEffect(() => {
    if (logged) !isEmpty(citys) && getDeliveryAddress();
      ...
      !isEmpty(citys) &&
        getDeliveryPrice(cityObj.id == 0 ? citys[0].id : cityObj.id);
    }
  }, [citys, cityObj]);
          ...
          .then(response => {
            ...
            if (response.status == 200) {
              if (response.result.length > 0) {
                let free_delivery =
                  response.result[0].free_delivery == null
                    ? 0
                    : response.result[0].free_delivery;
      ...
      <Animated.ScrollView
        ...
        {!success ? (
          <TouchableOpacity
            ...
            <View>
              <View style={styles.amount1}>
                <Text style={styles.priceGrey}>
                  ...
                  {(total > freeDelivery ? 0 : deliveryPrice).toFixed(2)}{' '}
                  {i18n.translate('lei')}
                </Text>
              </View>
              <View style={styles.amount1}>
                <Text style={styles.price}>
                  ...
                  <Text
                    style={
                      couponActive == 1 ? styles.couponPrice : styles.price
                    }>
                    {(
                      total + (total > freeDelivery ? 0 : deliveryPrice)
                    ).toFixed(2)}{' '}
                    {i18n.translate('lei')}
                  </Text>
                  {couponActive == 1 && (
            ...
            {minimumOrderPrice > finalPrice && (
              <View style={styles.notDeliveryBack}>
                ...
                <Text style={styles.notDelivery}>
                  {i18n.translate('Restaurant minimum order')}
                  {': '}
                  {minimumOrderPrice} {i18n.translate('lei')}
                </Text>
              </View>
            )}
            {!logged && (
                  ...
                  <Fragment>
                    {isDelivery == 0 && (
                      <View style={styles.notDeliveryBack}>
                        ...
                        <Text style={styles.notDelivery}>
                          {i18n.translate(
                            'Restaurant does not delivery in here',
                          )}
                        </Text>
                      </View>
                    )}
            ...
            <View style={styles.PhoneAndName}>
              ...
              <View style={styles.couponCodeContainer}>
                ...
                <TouchableOpacity
                  ...
                  disabled={disabled || errorCouponCode || isEmpty(couponCode)}
              ...
              {errorCouponCode != '' && (
                <View style={styles.errorCouponCodeBack}>
                  <ErrorIcon />
                  <Text style={styles.errorCouponCode}>{errorCouponCode}</Text>
                </View>
              )}
              {successCouponCode != '' && (
            ...
            <Text style={[styles.cartText, {marginTop: 20}]} numberOfLines={1}>
            ...
            <TouchableOpacity
              ...
              <Icon
                type="material"
                name={payment == 1 ? 'radio-button-on' : 'radio-button-off'}
                color={payment == 1 ? colors.YELLOW.PRIMARY : colors.BLACK}
                size={20}
              />
              <Text style={styles.radioText} numberOfLines={1}>
                {i18n.translate('Cash')}
            ...
            {!logged && (
              <TouchableOpacity
                ...
                onPress={() => setTermOfService(!termOfService)}>
                <Icon
                  type="material-community"
                  name={
                    termOfService
                  ...
                  size={25}
                  color={
                    termOfService ? colors.YELLOW.PRIMARY : colors.GREY.PRIMARY
                  }
                />
                <Text style={styles.rememberText}>
                  ...
                  <Text
                    ...
                    onPress={() =>
                      Linking.openURL('http://foodnet.ro/ro/terms')
                    }>
      ...
      {!success && (
        <View style={styles.goToOrder}>
          <View style={styles.orderAmount}>
            <Text style={styles.orderPrice}>{i18n.translate('Final')}</Text>
