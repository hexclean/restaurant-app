...
import {TrustIcon, CartYellowIcon} from '@constants/svgs';
        ...
        <Text style={styles.extraList}>
          ...
          {cartProduct.extras.map((extra, key) => (
            <Text key={`extra${key}`} style={styles.extra}>
              ...
              {key != cartProduct.extras.length - 1 ? ', ' : ''}
            </Text>
          ))}
        </Text>
      ) : null}
      <View style={styles.cartBottom}>
        <View style={styles.cartLeft}>
          <Text style={styles.price}>
            {cartProduct.productPrice.toFixed(2)} {i18n.translate('lei')}
          </Text>
          {!isEmpty(cartProduct.boxPrice) && (
            <Text style={styles.boxPrice}>
              {i18n.translate('Box price')}: {cartProduct.boxPrice.toFixed(2)}
              {i18n.translate('lei')}
            </Text>
          )}
        </View>
