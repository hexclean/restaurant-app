...
export default Detail = props => {
  ...
  useEffect(() => {
    const getFilterList = () => {
      ...
      if (filters.withinOneHour == 1)
        tempFilters = [
          ...tempFilters,
          {filter: i18n.translate('Within 1 hour')},
        ];
    ...
    const getInformation = () => {
      FoodService.information(country, restaurant.restaurant_name).then(
        response => {
  ...
  useEffect(() => {
    if (category === 0) {
      ...
      FoodService.categories(country, restaurant.restaurant_name, search)
        .then(async response => {
          if (response.status == 200) {
            ...
            var productsFinal = await Promise.all(
              productsTemp.map(async (productOne, key) => {
                var productList = await Promise.all(
                  productOne.product_list.map(async (product, key) => {
                    var allergens = await FoodService.allergen(
                      country,
                      product.productId,
      ...
      FoodService.products(
        ...
        .then(async response => {
          if (response.status == 200) {
            var productsFinal = await Promise.all(
              response.result.map(async (productOne, key) => {
                var productList = await Promise.all(
                  productOne.product_list.map(async (product, key) => {
                    var allergens = await FoodService.allergen(
                      country,
    ...
    FoodService.reviews(restaurant.restaurant_name, rating).then(response => {
      if (response.status == 200) {
  ...
  useEffect(() => {
    if (category === 0) {
      FoodService.categories(country, restaurant.restaurant_name, search).then(
        async response => {
          if (response.status == 200) {
            var productsTemp = [];
            await Object.keys(response.result).map(key => {
