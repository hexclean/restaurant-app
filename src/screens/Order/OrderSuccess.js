      ...
      <View key={`order${index}`} style={styles.cart}>
        ...
        {!isEmpty(orderItem.item.extras) ? (
          <Text style={styles.extraList}>
            +{' '}
            {orderItem.item.extras.map((extra, key) => (
              <Text key={`extra${key}`} style={styles.extra}>
                {extra.extra_quantity}*{extra.extra_name}
                {key != orderItem.item.extras.length - 1 ? ', ' : ''}
              </Text>
            ))}
          </Text>
        ) : null}
        <View style={styles.cartBottom}>
          <View style={styles.cartLeft}>
            <Text style={styles.price}>
              {orderItem.item.total_product_price} {i18n.translate('lei')}
            </Text>
          </View>
        </View>
      </View>
    ...
    <Container style={common.container}>
      ...
      <Header style={common.header}>
        <View style={common.headerLeft}>
          <TouchableOpacity
            onPress={() => props.navigation.navigate('App', {screen: 'Order'})}>
            <Icon
              type="material"
              name="arrow-back"
              size={30}
      ...
      <Content style={{flex: 1, padding: 20}}>
        <View
          ...
          <View style={{width: '100%', alignItems: 'center', padding: 20}}>
            ...
            <Text style={{fontSize: 16, color: '#111', marginTop: 20}}>
              {i18n.translate('Order placed')}:
            </Text>
            <Text
              style={{
                fontSize: 16,
            ...
            <Text style={{fontSize: 16, color: '#111', marginTop: 20}}>
              {i18n.translate('Place and method of collection')}:
            </Text>
            <Text
              style={{
                fontSize: 16,
              ...
              {!isEmpty(order.deliveryAddress) &&
                order.deliveryAddress[0].door_number}{' '}
              {!isEmpty(order.deliveryAddress) &&
                order.deliveryAddress[0].floor}{' '}
              {!isEmpty(order.deliveryAddress) &&
                order.deliveryAddress[0].house_number}
              {', '}
              {!isEmpty(order.deliveryAddress) &&
