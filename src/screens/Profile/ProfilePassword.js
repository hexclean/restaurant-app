...
export default ProfilePassword = props => {
  ...
  useEffect(() => {
    setErrorMsg('');
    (visitOldPassword && isEmpty(oldPassword)) ||
    (visitOldPassword && !validatePassword(oldPassword))
      ? setErrorOldPassword(
          i18n.translate('The password must be at least 6 characters long'),
        )
      : setErrorOldPassword('');
    (visitNewPassword && isEmpty(newPassword)) ||
    (visitNewPassword && !validatePassword(newPassword))
      ? setErrorNewPassword(
          i18n.translate('The password must be at least 6 characters long'),
        )
      : setErrorNewPassword('');
    (visitConfirmPassword && isEmpty(confirmPassword)) ||
    (visitConfirmPassword && !validatePassword(confirmPassword))
      ? setErrorConfirmPassword(
          i18n.translate('The password must be at least 6 characters long'),
        )
      : confirmPassword.length >= 5 && newPassword !== confirmPassword
      ? setErrorConfirmPassword(
          i18n.translate('The two passwords do not match'),
        )
      : setErrorConfirmPassword('');
  }, [
    oldPassword,
  ...
  const onChange = () => {
    ...
    ProfileService.modifyProfilePassword(
      user.token,
      oldPassword,
      newPassword,
      confirmPassword,
      ...
      <Content style={styles.content}>
        {!isEmpty(errorMsg) && (
          <View style={common.errorContainer}>
            <ErrorIcon />
            <Text style={{fontWeight: 'bold', color: '#F05050'}}>
              {errorMsg}
            </Text>
        ...
        <View style={[styles.inputView, common.marginTop25]}>
          <View style={common.flexRow}>
            <Text
              style={[
                styles.labelText,
                !isEmpty(errorOldPassword)
                  ? common.fontColorRed
                  : common.fontColorBlack,
              ]}>
              {i18n.translate('Old password')}
            </Text>
            <Text
              style={[
        ...
        <View style={[styles.inputView, common.marginTop25]}>
          <View style={common.flexRow}>
            <Text
              style={[
                styles.labelText,
                !isEmpty(errorNewPassword)
                  ? common.fontColorRed
                  : common.fontColorBlack,
              ]}>
              {i18n.translate('New password')}
            </Text>
            <Text
              style={[
                styles.labelTextNormal,
